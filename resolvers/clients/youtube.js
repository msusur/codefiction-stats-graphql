...
class YoutubeClient {
	...
	getChannel() {
		return google
			.youtube('v3')
			.channels.list({ part: 'statistics', key: youtube_config.key, id: youtube_config.channel_id })
			.then((channels) => {
	...
	getVideos(channelId, maxCount) {
		return google
			.youtube('v3')
			.channels.list({ part: 'contentDetails', key: youtube_config.key, id: channelId })
			.then((channels) => {
